...
import * as cheerio from 'cheerio'; //library that parse html for datascriping
...
export async function scrapeAmazonProduct(url: string) {
  ...
  const username = String(process.env.BRIBRIGTH_DATA_USERNAME);
  const password = String(process.env.BRIBRIGTH_DATA_PASSWORD);
  const port = 22225;
  ...
  try {
    ...
    const currentPrice = extractPrice(
      ...
      $('#price_inside_buybox'), //take price from here
